...
export const ControlPage = {
    ...
    showSettingsPanel() {
        ...
        // 1. 带单位输入的元数据
        const settingsMeta = {
            'setting-initial-script-delay': {
                value: SETTINGS.INITIAL_SCRIPT_DELAY / 1000, unit: '秒',
            },
            'setting-auto-pause-delay': {
                value: SETTINGS.AUTO_PAUSE_DELAY_AFTER_ACTION / 1000,
                unit: '秒',
            },
            'setting-unresponsive-timeout': {
                value: SETTINGS.UNRESPONSIVE_TIMEOUT / 60000, unit: '分钟',
            },
            'setting-red-envelope-timeout': {
                value: SETTINGS.RED_ENVELOPE_LOAD_TIMEOUT / 1000, unit: '秒',
            },
            'setting-popup-wait-timeout': {
                value: SETTINGS.POPUP_WAIT_TIMEOUT / 1000,
                unit: '秒',
            },
            'setting-worker-loading-timeout': {
                value: SETTINGS.ELEMENT_WAIT_TIMEOUT / 1000, unit: '秒',
            },
            'setting-close-tab-delay': {
                value: SETTINGS.CLOSE_TAB_DELAY / 1000,
                unit: '秒',
            },
            'setting-api-retry-delay': {
                value: SETTINGS.API_RETRY_DELAY / 1000,
                unit: '秒',
            },
            'setting-switching-cleanup-timeout': {
                value: SETTINGS.SWITCHING_CLEANUP_TIMEOUT / 1000, unit: '秒',
            },
            'setting-healthcheck-interval': {
                value: SETTINGS.HEALTHCHECK_INTERVAL / 1000, unit: '秒',
            },
            'setting-disconnected-grace-period': {
                value: SETTINGS.DISCONNECTED_GRACE_PERIOD / 1000, unit: '秒',
            },
        };
        ...
        const createUnitInput = (id, label) => {
            ...
            return `
                    <div class="qmx-settings-item">
                        <label for="${id}">
                            ${label}
                            <span class="qmx-tooltip-icon" data-tooltip-key="${id.replace(
                'setting-', '')}">?</span>
                        </label>
                        ...
                        <div class="qmx-settings-item">
                            <div class="theme-switch-wrapper">
                                <label class="theme-switch">
                                    <input type="checkbox" id="setting-theme-mode" ${SETTINGS.THEME ===
        'dark' ? 'checked' : ''}>
                        ...
                        <div class="qmx-settings-grid">
                            ...
                            <div class="qmx-settings-item">
                                ...
                                <label class="qmx-toggle">
                                    <input type="checkbox" id="setting-auto-pause" ${SETTINGS.AUTO_PAUSE_ENABLED
            ? 'checked'
            : ''}>
                            ...
                            <div class="qmx-settings-item">
                                ...
                                <div class="qmx-select" data-target-id="setting-daily-limit-action">
                                    ...
                                    <select id="setting-daily-limit-action" style="display: none;">
                                        <option value="STOP_ALL" ${SETTINGS.DAILY_LIMIT_ACTION ===
        'STOP_ALL' ? 'selected' : ''}>直接关停所有任务</option>
                                        <option value="CONTINUE_DORMANT" ${SETTINGS.DAILY_LIMIT_ACTION ===
        'CONTINUE_DORMANT' ? 'selected' : ''}>进入休眠模式，等待刷新</option>
                                    </select>
                                </div>
                            </div>
                            <div class="qmx-settings-item">
                                ...
                                <div class="qmx-select" data-target-id="setting-modal-mode">
                                    ...
                                    <select id="setting-modal-mode" style="display: none;">
                                        <option value="floating" ${SETTINGS.MODAL_DISPLAY_MODE ===
        'floating' ? 'selected' : ''}>浮动窗口</option>
                                        <option value="centered" ${SETTINGS.MODAL_DISPLAY_MODE ===
        'centered' ? 'selected' : ''}>屏幕居中</option>
                                        <option value="inject-rank-list" ${SETTINGS.MODAL_DISPLAY_MODE ===
        'inject-rank-list' ? 'selected' : ''}>替换排行榜显示</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                    ...
                    <div id="tab-perf" class="tab-content">
                        <div class="qmx-settings-grid">
                            ${createUnitInput('setting-initial-script-delay',
            '脚本初始启动延迟')}
                            ${createUnitInput('setting-auto-pause-delay',
            '领取后暂停延迟')}
                            ${createUnitInput('setting-unresponsive-timeout',
            '工作页失联超时')}
                            ${createUnitInput('setting-red-envelope-timeout',
            '红包活动加载超时')}
                            ${createUnitInput('setting-popup-wait-timeout',
            '红包弹窗等待超时')}
                            ${createUnitInput('setting-worker-loading-timeout',
            '播放器加载超时')}
                            ${createUnitInput('setting-close-tab-delay',
            '关闭标签页延迟')}
                            ${createUnitInput(
            'setting-switching-cleanup-timeout', '切换中状态兜底超时')}
                            ${createUnitInput('setting-healthcheck-interval',
            '哨兵健康检查间隔')}
                            ${createUnitInput(
            'setting-disconnected-grace-period', '断开连接清理延迟')}
                            ...
                            <div class="qmx-settings-item" style="grid-column: 1 / -1;">
                                ...
                                <div class="qmx-range-slider-wrapper">
                                    <div class="qmx-range-slider-container">
                                        ...
                                        <input type="range" id="setting-min-delay" min="0.1" max="5" step="0.1" value="${SETTINGS.MIN_DELAY /
        1000}">
                                        <input type="range" id="setting-max-delay" min="0.1" max="5" step="0.1" value="${SETTINGS.MAX_DELAY /
        1000}">
                                    </div>
                    ...
                    <div id="tab-advanced" class="tab-content">
                        <div class="qmx-settings-grid">
                            ...
                            ${createUnitInput('setting-api-retry-delay',
            'API重试延迟')}
        ...
        modal.addEventListener('mouseover', (e) => {
            ...
            if (allTooltips[key]) {
                ...
                globalTooltip.style.left = `${iconRect.left + iconRect.width /
                2}px`;
        ...
        modal.querySelectorAll('.qmx-select').forEach(wrapper => {
            ...
            optionsList.querySelectorAll('div').forEach(optionDiv => {
                optionDiv.addEventListener('click', () => {
                    ...
                    optionsList.querySelector('.selected')?.
                        classList.
                        remove('selected');
        ...
        document.addEventListener('click',
            () => modal.querySelectorAll('.qmx-select.active').
        ...
        function updateSliderView() {
            ...
            const minPercent = ((minSlider.value - minSlider.min) /
                (minSlider.max - minSlider.min)) * 100;
            const maxPercent = ((maxSlider.value - maxSlider.min) /
                (maxSlider.max - minSlider.min)) * 100;
        ...
        modal.querySelector(
            '#qmx-settings-cancel-btn').onclick = () => this.hideSettingsPanel();
        modal.querySelector(
            '#qmx-settings-save-btn').onclick = () => this.saveSettings();
        ...
        modal.querySelectorAll('.tab-link').forEach(button => {
            button.onclick = (e) => {
                ...
                modal.querySelectorAll('.tab-link.active').
                    forEach(btn => btn.classList.remove('active'));
                modal.querySelectorAll('.tab-content.active').
    ...
    saveSettings() {
        ...
        const newSettings = {
            // Tab 1: 基本设置
            CONTROL_ROOM_ID: document.getElementById(
                'setting-control-room-id').value,
            AUTO_PAUSE_ENABLED: document.getElementById(
                'setting-auto-pause').checked,
            DAILY_LIMIT_ACTION: document.getElementById(
                'setting-daily-limit-action').value,
            MODAL_DISPLAY_MODE: document.getElementById(
                'setting-modal-mode').value,
            THEME: document.getElementById('setting-theme-mode').checked
                ? 'light'
                : 'dark', // 保存主题设置

            // Tab 2: 性能与延迟 (单位转换：从 秒/分钟 转为 毫秒)
            INITIAL_SCRIPT_DELAY: parseFloat(
                    document.getElementById('setting-initial-script-delay').value) *
                1000,
            AUTO_PAUSE_DELAY_AFTER_ACTION: parseFloat(
                    document.getElementById('setting-auto-pause-delay').value) *
                1000,
            SWITCHING_CLEANUP_TIMEOUT: parseFloat(
                    document.getElementById(
                        'setting-switching-cleanup-timeout').value) *
                1000,
            UNRESPONSIVE_TIMEOUT: parseInt(
                    document.getElementById('setting-unresponsive-timeout').value,
                    10) *
                60000,
            RED_ENVELOPE_LOAD_TIMEOUT: parseFloat(
                    document.getElementById('setting-red-envelope-timeout').value) *
                1000,
            POPUP_WAIT_TIMEOUT: parseFloat(
                    document.getElementById('setting-popup-wait-timeout').value) *
                1000,
            ELEMENT_WAIT_TIMEOUT: parseFloat(
                document.getElementById(
                    'setting-worker-loading-timeout').value) * 1000,
            MIN_DELAY: parseFloat(
                document.getElementById('setting-min-delay').value) * 1000,
            MAX_DELAY: parseFloat(
                document.getElementById('setting-max-delay').value) * 1000,
            CLOSE_TAB_DELAY: parseFloat(
                    document.getElementById('setting-close-tab-delay').value) *
                1000,
            HEALTHCHECK_INTERVAL: parseFloat(
                    document.getElementById('setting-healthcheck-interval').value) *
                1000,
            DISCONNECTED_GRACE_PERIOD: parseFloat(
                    document.getElementById(
                        'setting-disconnected-grace-period').value) *
                1000,

            // Tab 3: 高级设置
            MAX_WORKER_TABS: parseInt(
                document.getElementById('setting-max-tabs').value, 10),
            API_ROOM_FETCH_COUNT: parseInt(
                document.getElementById('setting-api-fetch-count').value, 10),
            API_RETRY_COUNT: parseInt(
                document.getElementById('setting-api-retry-count').value, 10),
            API_RETRY_DELAY: parseFloat(
                    document.getElementById('setting-api-retry-delay').value) *
                1000,
        };
    ...
    bindEvents() {
        ...
        document.getElementById(
            'qmx-modal-settings-btn').onclick = () => this.showSettingsPanel();

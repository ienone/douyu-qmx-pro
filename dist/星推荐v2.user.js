...
showSettingsPanel() {
      ...
      const settingsMeta = {
        "setting-initial-script-delay": {
          value: SETTINGS.INITIAL_SCRIPT_DELAY / 1e3,
          unit: "秒"
        },
        "setting-auto-pause-delay": {
          value: SETTINGS.AUTO_PAUSE_DELAY_AFTER_ACTION / 1e3,
          unit: "秒"
        },
        "setting-unresponsive-timeout": {
          value: SETTINGS.UNRESPONSIVE_TIMEOUT / 6e4,
          unit: "分钟"
        },
        "setting-red-envelope-timeout": {
          value: SETTINGS.RED_ENVELOPE_LOAD_TIMEOUT / 1e3,
          unit: "秒"
        },
        "setting-popup-wait-timeout": {
          value: SETTINGS.POPUP_WAIT_TIMEOUT / 1e3,
          unit: "秒"
        },
        "setting-worker-loading-timeout": {
          value: SETTINGS.ELEMENT_WAIT_TIMEOUT / 1e3,
          unit: "秒"
        },
        "setting-close-tab-delay": {
          value: SETTINGS.CLOSE_TAB_DELAY / 1e3,
          unit: "秒"
        },
        "setting-api-retry-delay": {
          value: SETTINGS.API_RETRY_DELAY / 1e3,
          unit: "秒"
        },
        "setting-switching-cleanup-timeout": {
          value: SETTINGS.SWITCHING_CLEANUP_TIMEOUT / 1e3,
          unit: "秒"
        },
        "setting-healthcheck-interval": {
          value: SETTINGS.HEALTHCHECK_INTERVAL / 1e3,
          unit: "秒"
        },
        "setting-disconnected-grace-period": {
          value: SETTINGS.DISCONNECTED_GRACE_PERIOD / 1e3,
          unit: "秒"
        }
      };
      const allTooltips = {
                            ...
                            <div class="qmx-settings-item" style="grid-column: 1 / -1;">
                                ...
                                <div class="qmx-range-slider-wrapper">
                                    <div class="qmx-range-slider-container">
                                        ...
                                        <input type="range" id="setting-min-delay" min="0.1" max="5" step="0.1" value="${SETTINGS.MIN_DELAY / 1e3}">
                                        <input type="range" id="setting-max-delay" min="0.1" max="5" step="0.1" value="${SETTINGS.MAX_DELAY / 1e3}">
                                    </div>
      ...
      document.addEventListener(
        "click",
        () => modal.querySelectorAll(".qmx-select.active").forEach((el) => el.classList.remove("active"))
      );
      ...
      modal.querySelector(
        "#qmx-settings-cancel-btn"
      ).onclick = () => this.hideSettingsPanel();
      modal.querySelector(
        "#qmx-settings-save-btn"
...
INITIAL_SCRIPT_DELAY: parseFloat(
        ...
        MIN_DELAY: parseFloat(
          document.getElementById("setting-min-delay").value
        ) * 1e3,
        MAX_DELAY: parseFloat(
          document.getElementById("setting-max-delay").value
        ) * 1e3,
...
MAX_WORKER_TABS: parseInt(
          document.getElementById("setting-max-tabs").value,
          10
        ),
        API_ROOM_FETCH_COUNT: parseInt(
          document.getElementById("setting-api-fetch-count").value,
          10
        ),
        API_RETRY_COUNT: parseInt(
          document.getElementById("setting-api-retry-count").value,
          10
        ),

...
---
## 关于斗鱼新版界面的说明
斗鱼已更新直播间界面，脚本正在积极适配中。目前基本功能已恢复可用，但请注意以下已知问题与限制：

- **显示模式限制**：控制面板的 **“替换排行榜”** 模式暂不可用，请暂时使用 **“浮动窗口”** 或 **“屏幕居中”** 模式。
- **加载问题**：刚开始打开的几个工作标签页可能需要手动切换激活一下才能正常加载。
- **功能限制**：弹幕助手功能正在适配中，暂时无法使用。
- **插件冲突**：如需使用抢红包功能，请暂时关闭 **斗鱼Ex** 插件，否则可能导致红包消失。
- **网络设置**：请在浏览器设置中搜索 "DNS"，将 **“使用安全的 DNS”** 选项关闭，否则可能影响红包获取。
